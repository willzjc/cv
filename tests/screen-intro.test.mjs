...
test("intro uses saved identity content and works without visualization libraries", async (context) => {
  const fixture = await openFixture({
    withoutD3: true,
    localStorage: { "will-chen-resume-v10": JSON.stringify({ name: "Will Chen Test", headline: "Engineering Lead Test" }) }
  });
  context.after(fixture.close);
...
test("stale snapshots are ignored while current-version edits are preserved", async (context) => {
  ...
  assert.equal(stale.document.querySelector('[data-edit-id="headline"]').textContent, "Principal Software Engineer | AI-enabled Reliability & Platforms");
